					...
					<div class="login_part_form_iner">
						<form class="row contact_form" id="joinForm">
							...
							<div class="col-md-6 form-group">
								<label for="password">비밀번호 (영문, 숫자, 특수문자 8~16자)</label> <input
							...
							<!-- 생년월일 -->
							<div class="col-md-6 form-group">
								<label for="businessNum">사업자등록번호</label> <input type="text"
									id="businessNum" name="businessNum" class="form-control"
									required />
							</div>

							<!-- 휴대폰 -->
							<div class="col-md-12 form-group">
								<label for="tel">휴대폰 번호 (숫자만 입력)</label> <input type="text"
									id="tel" name="tel" class="form-control" required />
							</div>

							<div class="col-md-12 form-group">
								<label for="accountNum">정산 계좌번호 (숫자만 입력)</label> <input type="text"
									id="accountNum" name="accountNum" class="form-control" required />
							</div>

							<!-- 은행명 select 박스 -->
							<div class="col-md-12 form-group">
								<label for="bank">은행명</label> <select id="bank" name="bank"
									class="form-control" required>
								...
								<div class="row g-2">
									...
									<div class="col-5 col-sm-4">
										<input type="text" id="zipCode" name="zipCode"
											class="form-control" placeholder="우편번호" readonly required>
									</div>
									<div class="col-7 col-sm-4">
										<button type="button" class="btn-check"
											onclick="findZipCode()">우편번호 찾기</button>
									</div>
									<!-- 기본주소 -->
									<div class="col-12 mt-2">
										<input type="text" id="address" name="address"
											class="form-control" placeholder="기본 주소" readonly required>
									</div>
									<!-- 상세주소 -->
									<div class="col-12 mt-2">
							...
							<div class="col-md-12 form-group">
								<div class="form-check">
									<input class="form-check-input" type="checkbox" id="agree"
										required /> <label class="form-check-label" for="agree">이용약관
										및 개인정보 수집에 동의합니다. (필수)</label>
								</div>
							</div>
							...
							<div class="col-md-12 form-group">
								<button type="button" class="btn_3 w-100"
									onclick="submitJoinForm()">회원가입</button>
							</div>
						</form>
					</div>
				</div>
...
<script>
	...
	function checkEmailDuplicate() {
		...
		$.ajax({
			type : 'POST',
			url : '/store/seller/checkEmailDuplicate',
			data : {
				email : email
			},
			success : function() {
	...
	function submitJoinForm() {
		...
		$.ajax({
			type : 'POST',
			url : '/store/seller/join',
			data : $('#joinForm').serialize(),
			success : function(res) {
				if (res.status === 'ok') {
					alert(res.msg);
					location.href = '/store/seller/login';
				}
			},
			error : function(xhr) {
