								...
								<div class="product-price">
									<c:choose>
										<c:when test="${product.discount != 0}">
											<span class="discounted-price"> <fmt:formatNumber value="${product.price - product.discount}" type="number" />원
											</span>
											<span class="original-price"> <fmt:formatNumber value="${product.price}" type="number" />원
											</span>
										</c:when>
										<c:otherwise>
...
<script>
	...
	function renderProducts(products) {
		...
		if (products.length === 0) {
			container
					.append('<div class="col-12 text-center text-muted py-5">등록된 상품이 없습니다.</div>');
			return;
		}
		...
		products
				.forEach(function(product) {
					...
					if (product.discount !== 0) {
						priceHtml = '<span class="discounted-price">'
								+ (product.price - product.discount)
								+ '원</span>' + '<span class="original-price">'
					...
					var productHtml = '<div class="col product-item" data-product-id="' + product.productId + '">'
